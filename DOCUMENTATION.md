...
- **Bank A/Bank B pairing (next/prev moves both banks together)** — client asked for this; it was already implemented (`2026-09-07` entry below, `LaunchedEffect(currentKit) { currentKitB = ... }`). Verified still correct, no change.
- **Bank A direct patch load works; Bank B's didn't.** `targetSlotForLoad()` (used by "Load Kit From Folder" and "Import Patch") was hardcoded to Bank A's pool regardless of which bank was actually active — loading while Bank B was selected silently landed the kit in Bank A instead. Now bank-aware: `bankMode == "B"` targets `currentKitB` (or the first free Bank B slot) instead. Both call sites now go through a new `commitLoadedKitIndex()` helper that sets `currentKitB` instead of `currentKit` when the returned slot is actually in Bank B's range — assigning a Bank B index to `currentKit` would have silently re-paired Bank A onto a Bank B slot number via the pairing effect above, corrupting both banks. Also fixed the two screens' `DrumEngine.invalidatePad(padIndex)` calls (bare pad 0-7 index) to go through `nativeSlotsFor(padIndex)` — correct only for Bank A before, so a Bank-B-targeted load would previously have flagged the wrong native slot for reload.
